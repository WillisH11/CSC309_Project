    ...
    if (role !== undefined && role !== null) {
      ...
      if (req.auth.role === "manager") {
        ...
        if (!["regular", "cashier"].includes(role)) {
          return res
            .status(403)
            .json({ error: "Managers can only set regular or cashier roles" });
        }
...
app.post("/transactions", jwtMiddleware, async (req, res) => {
  try {
    ...
    if (
      type === "purchase" &&
      !["cashier", "manager", "superuser"].includes(req.auth.role)
    ) {
      return res.status(403).json({ error: "Forbidden" });
    }
    ...
    if (type === "purchase") {
      ...
      isSuspiciousTransaction =
        (cashier && cashier.suspicious) || user.suspicious;
      ...
      const activePromotions = await prisma.promotion.findMany({
        where: {
          ...
          startTime: { lte: now },
          endTime: { gte: now },
          OR: [{ minSpending: null }, { minSpending: { lte: spentAmount } }],
        },
      });
      ...
      if (
        ...
        if (onetimePromotions.length !== promotionIds.length) {
          return res
            .status(400)
            .json({ error: "One or more promotion IDs are invalid" });
        }
...
app.post("/users/me/transactions", jwtMiddleware, async (req, res) => {
  try {
    ...
    if (!user.verified) {
      return res
        .status(403)
        .json({ error: "User must be verified to redeem points" });
    }
...
app.patch("/events/:eventId", jwtMiddleware, async (req, res) => {
  try {
    ...
    if (
      (req.body.points !== undefined || req.body.published !== undefined) &&
      !["manager", "superuser"].includes(req.auth.role)
    ) {
      return res.status(403).json({ error: "Forbidden" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Cannot update these fields after event has started" });
    }
    ...
    if (endTime !== undefined && endTime !== null) {
      if (eventHasEnded) {
        return res
          .status(400)
          .json({ error: "Cannot update end time after event has ended" });
      }
      const end = new Date(endTime);
    ...
    if (points !== undefined && points !== null) {
      ...
      if (newPointsRemain < 0) {
        return res
          .status(400)
          .json({ error: "Cannot reduce points below already awarded amount" });
      }
...
app.delete("/events/:eventId", jwtMiddleware, async (req, res) => {
  try {
    ...
    if (event.pointsAwarded > 0) {
      return res
        .status(400)
        .json({ error: "Cannot delete event with awarded points" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (event.organizers.length === 1) {
        return res
          .status(400)
          .json({ error: "Cannot remove the last organizer" });
      }
...
app.post("/events/:eventId/transactions", jwtMiddleware, async (req, res) => {
  try {
    ...
    if (!utorid) {
      ...
      guestIds = event.guests
        .filter((g) => g.confirmedAt !== null)
        .map((g) => g.userId);
    } else {
    ...
    if (event.pointsRemain < totalPointsNeeded) {
      return res
        .status(400)
        .json({ error: "Insufficient points remaining for this event" });
    }
  ...
  try {
    ...
    if (type === "automatic") {
      ...
      if (rate === undefined || rate === null || isNaN(rate) || rate < 0) {
        return res
          .status(400)
          .json({ error: "Automatic promotions require a valid rate" });
      }
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "One-time promotions require valid points" });
      }
      ...
      if (!minSpending || isNaN(minSpending) || minSpending <= 0) {
        return res
          .status(400)
          .json({ error: "One-time promotions require minimum spending" });
      }
  ...
  try {
    ...
    if (rate !== undefined) {
      if (promotion.type !== "automatic") {
        return res
          .status(400)
          .json({ error: "Cannot set rate for non-automatic promotions" });
      }
    ...
    if (points !== undefined) {
      if (promotion.type !== "onetime") {
        return res
          .status(400)
          .json({ error: "Cannot set points for non-onetime promotions" });
      }
